...
model {
  ...
  alpha_grm ~ normal(0, sigma_alpha_grm);
  ...
  for(i in 1:N){
    for (j in 1:p){
       	...
       	pq=inv_logit(alpha_pl[j]*(theta[i,1] - beta_pl[j])); 
       	Y[i,j] ~ bernoulli(pq);
      }
  }
  // now estimate the GRM portion here
  for(i in 1:NG) {
    y[i] ~ ordered_logistic(theta[jj[i],2]*alpha_grm[kk[i]],kappa[kk[i]]);
  }
